...
서버 측은 `packages/udp-service` (UDP gateway, 기본 port `3001`).
...
2. PC 에서 서버 기동 (Docker compose 또는 직접 `yarn workspace @lostark/udp-gateway start`).
   - 방화벽이 `3001/udp` 인바운드를 막지 않는지 확인 (Windows Defender 인바운드
     규칙).
