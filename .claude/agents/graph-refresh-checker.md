...
Yarn Berry PnP + 4-package 모노레포 + Fastify REST 구조. 별도 지정이 없다면 다음 scope 가 자연스럽다:
...
5. scope 별로 다음을 계산한다:
   ...
   - import/export, 라우트 정의, 스키마 정의 같은 **구조적 시그널** 이 포함됐는지 — 본 레포에서 참고할 패턴:
     - `^export\s+(class|function|const|interface|type)` (공개 API 경계)
     - `(fastify|app|server)\.(get|post|put|delete|patch|route)\(` (Fastify 라우트 정의)
     - `\.register\(` (Fastify 플러그인 등록)
     - `schema:\s*{` 블록 (REST 계약 변경 후보)
     - `createPool\(|createConnection\(|createClient\(` (MySQL/Redis 연결 지점 변경)
...
- **fully-stale**: 여러 scope 에 걸쳐 구조적 시그널이 많이 발생, 혹은 파일 추가/삭제 10건 이상. 전체 재생성 권고. 패키지 간 의존(`@lostark/*` import) 이 새로 생기거나 사라지면 거의 항상 이 판정.
...
- scope 가 특정 패키지 전용이었고 해당 패키지가 제거/통합되었다면 삭제 권고 (호출자가 맥락으로 판단할 수 있게 사유 서술).
