...
> **캐시 무효화 안내**: `NormalizedCharacterDetail` JSON 에 `abilityStone`
> 필드가 추가됨. 기존 캐시 entry 는 `abilityStone: undefined` → formatter 가
> null 로 처리 (graceful degradation). 즉시 신 응답을 원하면 Redis `FLUSHDB`
> 또는 캐시 prefix 무효화 가능 (선택).
...
- `yarn workspace @lostark/rest-api dump:openapi` 실행 후 LoA-Bot generated
  types 재생성 권고 (파괴적 조작 게이트 통과 후)
- `should handle API errors gracefully` (`armories.test.ts:86`) assertion 버그
  수정 — 별도 fix
...
- [analysis: armories legacy vs 현재 formatter 출력 필드 비교](../analysis/armories-legacy-vs-current-output-fields.md)
  — 본 재기획으로 역전. `superseded_note` 참조
- [changes: F7 armories !정보 legacy parity](./2026-05-16-armories-info-legacy-parity.md)
  — 본 세션이 재기획한 대상
