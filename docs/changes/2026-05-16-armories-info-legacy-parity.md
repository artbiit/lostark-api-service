...
`NormalizedCharacterDetail` 도메인 모델에 ArkPassive / characterLevel /
guildMemberGrade 필드를 추가하고, V9 타입 3건을 실응답 기반으로 정정, formatter
9개 출력 라인을 추가했다.
...
> `loa-platform` 프로젝트의 `generated.ts` 를
> 재생성(`yarn workspace @lostark/rest-api dump:openapi` 후 클라이언트
> codegen)할 것.
...
`packages/data-service/src/services/armories-service.ts`
