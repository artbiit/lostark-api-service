...
## graphify 버전 카운트 드리프트 주의

동일 코드베이스라도 **graphify 설치 버전이 바뀌면 node/edge/community 카운트가
달라질 수 있다**. ADR-0004 세션에서 packages scope 를 재생성했을 때, 직전 메타
(594 nodes·17 communities @`26a49b1`, 이전 버전의 ghost-dedup 산출)와 달리 현재
설치 버전(python3.14 env)이 동일 코드에서 849 nodes·48 communities 를 안정적으로
산출했다(fresh-cache 재현으로 stale-cache 오염이 아님을 확인 — 순수 버전/방법론
드리프트). 함의:

- `graph-refresh-checker` 의 카운트 기반 staleness 판정 및 메타 표의 커밋 간
  카운트 비교는 **동일 graphify 버전 내에서만** 신뢰할 수 있다.
- 버전이 바뀐 직후의 카운트 급변은 코드 회귀가 아닐 수 있다 — 재생성 시 메타에
  버전 드리프트를 명시하고, 가능하면 `docs/graph/index.md` 에 graphify 버전을
  함께 기록해 re-baseline 근거를 남긴다.
