...
- **역할**: UDP 메시지 처리
- **포트**: 5022 (환경변수 `UDP_GATEWAY_PORT`로 변경 가능)
- **프로필**: `udp`
...
- **인스턴스**: Kord 프로젝트의 `kord-postgres` 컨테이너를 외부
  네트워크(`kord_default`)로 조인하여 재사용
- **사전 준비**:
...
- `UDP_GATEWAY_PORT`: UDP Gateway 포트 (기본값: 5022)
