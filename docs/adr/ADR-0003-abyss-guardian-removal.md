...
`!도비스` / `!도가토` 명령과 이에 연동된 formatter, 테스트를 코드베이스에서 완전
제거한다.
...
- `CALENDAR_CATEGORIES` 에서 미사용 키 정리 — category-map 상수가 실제 API
  응답과 일치.
- 연동 테스트에서 항상 실패할 소지가 있던 smoke 케이스 2건 제거.

**부정적 / 위험**

- 향후 게임이 해당 콘텐츠를 재도입할 경우 명령을 다시 추가해야 한다. 단, ADR 과
  work-log 에 삭제 사실이 기록되어 있어 재구현 맥락 파악이 용이함.
- `docs/work-log/2026-05-16-udp-service-kakao-bot-promotion/index.md` 에 기재된
  27종 명령 목록은 해당 시점의 사실 기록이므로 수정하지 않는다. 향후
  architecture 문서(`docs/architecture/udp-service-kakao-bot.md`) 갱신은
