...
로스트아크 원격 API (developer-lostark.game.onstove.com) 를 **Fastify 기반 HTTP 서비스로 래핑 · 캐싱 · 재노출** 하는 독립 서비스. 여러 소비자(Discord 봇, 웹 대시보드 등) 의 공통 백엔드 역할을 한다.

**docs-first 워크플로우**: 어떤 작업이든 이 파일을 먼저 읽고, 해당 카테고리의 `index.md` 를 거쳐 구체 문서로 들어간다.
...
- [ ] 구조 탐색이 필요한 작업이라면 `graph/index.md` 의 last_generated_at 이 최근인지 확인했는가? 오래됐다면 `graph-refresh-checker` 에이전트 호출 판단.
