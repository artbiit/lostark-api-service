...
`packages/udp-service` 가 카카오톡 명령어 포맷팅 봇으로 승격된 이후의
모듈 토폴로지·envelope 계약·라우팅 흐름을 기록한다.
...
카카오톡 명령어를 처리하고 plain-text 응답을 반환하는 내부망 전용 봇 서비스.
API 호출은 `packages/data-service` 의 서비스 싱글톤을 통해 위임한다.
...
3. 핸들러 내부 throw → 카카오 친화적 오류 메시지로 reply (`"<name> 은(는) 없는 것 같숨미당"`).
...
기존 UdpMessage 4종(`character_detail`, `character_refresh`, `cache_status`, `ping`)은
dead code 로 판정·폐기. → [ADR-0001](../adr/ADR-0001-udp-envelope-adoption.md)
