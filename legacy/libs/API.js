...
async function news_notices(searchText, type) {
  return await request('news/notices', { searchText: searchText, type: type }, methods.get);
}
...
async function character_siblings(characterName) {
  const job = await request(`characters/${characterName}/siblings`, {}, methods.get);
  return job;
}
...
async function armories_character(characterName, type) {
  ...
  const job = await request(`armories/characters/${characterName}/${type}`, {}, methods.get);
  return job;
}
/** 경매장 검색 옵션을 반환합니다. */
