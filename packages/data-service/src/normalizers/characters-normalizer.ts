...
import {
  ...
  findMostActiveServer,
  generateAccountId,
  parseItemLevel,
} from '@lostark/shared/types/domain';
...
export class CharactersNormalizer {
  ...
  async normalizeSiblings(
    ...
    // 캐릭터 정보 변환
    const characters: CharacterInfo[] = siblingsData.map((sibling: any) => ({
  ...
  async detectChanges(
    ...
    const deletedCharacters: CharacterInfo[] = [];

    // 새 캐릭터 정보 변환
    const newCharacterMap = new Map<string, CharacterInfo>();
    ...
    for (const newChar of newCharacterMap.values()) {
      const existingChar = currentAccount.characters.find(
        (char: any) => char.characterName === newChar.characterName,
      );
    ...
    const detection: CharacterChangeDetection = {
      changes,
      newCharacters,
      deletedCharacters,
      accountId: currentAccount.accountId,
  ...
  async updateAccountInfo(
    ...
    const newAccountInfo = await this.normalizeSiblings(
      currentAccount.characters[0]?.characterName || 'unknown',
      newSiblingsData,
    );
    ...
    const deletedCharacterNames = currentAccount.characters
      .filter(
        (char: any) =>
          !newSiblingsData.find((sibling: any) => sibling.CharacterName === char.characterName),
      )
   ...
   * ARMORIES 큐 항목 생성
   */
  generateArmoriesQueueItems(
    detection: CharacterChangeDetection,
  ): Array<{ characterName: string; reason: string; priority: number }> {
    const queueItems: Array<{ characterName: string; reason: string; priority: number }> = [];
